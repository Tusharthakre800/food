...
const Comment = ({ item, containerRef }) => {
    ...
    return (
        <>
            <button onClick={toggleComments} className="flex flex-col items-center gap-1 group">
            ...
            {showComments && containerRef?.current && createPortal(
                 <div className="absolute bottom-0 left-0 w-full h-[70vh] bg-black/90 backdrop-blur-xl rounded-t-3xl z-40 flex flex-col shadow-2xl border-t border-white/10 transition-transform duration-300 ease-in-out" onClick={(e) => e.stopPropagation()}>
                    ...
                    <form onSubmit={handlePostComment} className="p-4 border-t border-white/10 flex gap-2">
                        ...
                        />
                        <button type="submit" disabled={!commentText.trim()} className="text-primary font-semibold disabled:opacity-50 px-2">Post</button>
                    </form>
                 </div>,
